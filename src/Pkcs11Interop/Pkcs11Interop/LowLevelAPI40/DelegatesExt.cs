...
namespace Net.Pkcs11Interop.LowLevelAPI40
{
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    internal delegate uint C_EX_InitTokenDelegate(uint slotId, byte[] pin, uint pinLen, CK_RUTOKEN_INIT_PARAM initInfo);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    internal delegate uint C_EX_GetTokenNameDelegate(uint session, ref byte[] tokenName, ref uint tokenNameLen);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    internal delegate uint C_EX_SetLicenseDelegate(uint session, uint licenseNume, byte[] license, uint licenseLen);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    internal delegate uint C_EX_GetLicenseDelegate(uint session, uint licenseNume, ref byte[] license, ref uint licenseLen);
